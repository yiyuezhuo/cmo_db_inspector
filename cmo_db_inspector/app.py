...
class SelectorTab:
    ...
    def build(self):
        with gr.Tab("Selector"):
            with gr.Row():
                with gr.Column():
                    with gr.Row():
                        self.cmd_dababase_dropdown = gr.Dropdown([p.name for p in self.db_list], label="CMO Database", value=self.default_db.name)
        ...
        inputs = {self.cmd_dababase_dropdown, self.type_dropdown, self.page_index_number, self.class_text}
        ...
        self.gr_df.select(self.select, {self.gr_df, self.type_dropdown}, self.gr_df_select_output)
    ...
    def select(self, data, evt: gr.SelectData):
        ...
        event.process_callback(_id)
        
        self.gr_df_select_output.clear()
        self.gr_df_select_output.extend(event.outputs)
        
        return event.returns
    
    def update(self, data, page_target=None, page_offset=None):
        db_name = data[self.cmd_dababase_dropdown]
